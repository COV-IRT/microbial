...
tib2
tib2<-tib2%>%pivot_longer(c(COVID_19vsCommunity_acquired_pneumonia,COVID_19vsUninfected),names_to="comparison",values_to="wilcox_p_value")
...
tail(tib3)
vector<-tally(~species,tib3,"data.frame")%>%arrange(desc(Freq))
tib3
vector
tib4<-tib3%>%filter(species%in%vector$species)
tib4
#tib4<-tib4%>%select(-c(taxon_id,otu_id))%>%filter(!is.na(species))
#tib4<-tib4%>%select(-species)%>%distinct()
tib4
tib4%>%arrange(species,wilcox_p_value)
unique(tib4$species)
...
tib<-tib%>%filter(!is.na(species))%>%distinct_all()
...
tib5<-tib3%>%
  filter(species%in%vector$species)%>%filter(!is.na(species))
round(range(tib5$bh_q_val),digits = 2)
#get rid of the species names and make the genera distinct
# and arrange them by the most significant
tib5<-tib5%>%distinct_all()%>%arrange(species,desc(bh_q_val,wilcox_p_value))
unique(tib5$species)
unique(tib5$log2_median_ratio)
unique(tib5$mean_diff)
unique(tib5$median_diff)
unique(tib5$wilcox_p_value)
as.data.frame(tib5)
tib5$species
write.table(x = tib5,
            file = "top_taxa_assoc_w_SARS_COV2_infection_full.tsv",
            sep = "\t",
            row.names = F)
