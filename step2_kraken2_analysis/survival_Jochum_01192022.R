...
survival_pseq_decontam_no_neg_core
...
df_input_metadata%>%select(patient,survival)
#########################################
...
physeq_keep<-prune_taxa(taxa = keep$feature, x =  survival_pseq_decontam_no_neg_core)
...
pseq_obj_filt$all_names()
names$taxon_id<-rownames(names)
#merge the two datasets together
sig<-inner_join(pseq_obj_filt$data$diff_table_sig,names)
sig

key<-pseq_obj_filt$data[['otu_table']]%>%select(taxon_id,otu_id)
key


sig<-left_join(sig,key)#%>%select(-c(taxon_id,`pseq_obj_filt$taxon_names()`))
sig
sig<-sig%>%
  rename(OTU=otu_id)%>%
  distinct_all()
