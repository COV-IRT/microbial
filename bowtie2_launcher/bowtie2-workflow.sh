...
if [[ "$pairedEnd" == "true" ]]; then
    ...
    bowtie2 \
        -x /scratch/06176/jochum00/COVIRT19/7nrd3/osfstorage/GRCh38/Homo_sapiens/NCBI/GRCh38/Sequence/Bowtie2Index/genome \
        -1 $forwardReads \
        -2 $reverseReads \
        -p $threads \
        --very-sensitive \
        --un-conc-gz $PWD/bowtie2_out/unaligned/$fileBasename \
        --al-conc-gz $PWD/bowtie2_out/aligned/$fileBasename \
        -S $PWD/bowtie2_out/SAM/${fileBasename}.sam

    echo "LOG: running samtools sam to bam"
    samtools view --threads $threads -f 4 -bS $PWD/bowtie2_out/SAM/${fileBasename}.sam > $PWD/bowtie2_out/BAM/${fileBasename}.unmapped.bam
#    samtools view --threads $threads -bS $PWD/bowtie2_out/SAM/${fileBasename}.unmapped.sam > $PWD/bowtie2_out/BAM/${fileBasename}.unmapped.bam
    ...
    bowtie2 \
        ...
        -p $threads \
        --very-sensitive \
        --un-gz $PWD/bowtie2_out/unaligned/$fileBasename \
        --al-gz $PWD/bowtie2_out/aligned/$fileBasename \
        -S $PWD/bowtie2_out/SAM/${fileBasename}.SE.sam
    
    echo "LOG: running samtools sam to bam"
    samtools view -f 4 --threads $threads -bS $PWD/bowtie2_out/SAM/${fileBasename}.SE.sam > $PWD/bowtie2_out/BAM/${fileBasename}.unmapped.SE.bam
#    samtools view --threads $threads -bS $PWD/bowtie2_out/SAM/${fileBasename}.unmapped.SE.sam > $PWD/bowtie2_out/SAM/${fileBasename}.unmapped.SE.bam
